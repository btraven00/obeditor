...
const PREVIEW_PY = [
  ...
  "            commit = (module.repository.commit[:8]",
  "                      if len(module.repository.commit) >= 8",
  "                      else module.repository.commit)",
  "            for combo in _all_combos(module):",
  "                ph = _phash(combo)",
  ...
  "                # params",
  "                out.write('    params:\\n')",
  "                out.write(f'        module_dir=\".modules/{commit}/\",\\n')",
  ...
  "    out.write('    local name=\"$1\" url=\"$2\" commit=\"$3\"\\n')",
  "    out.write('    local dest=\"$MODULES_DIR/$commit\"\\n')",
  "    out.write('    if [ -d \"$dest/.git\" ]; then\\n')",
  "    out.write('        echo \"  checkmark $name already resolved\"\\n')",
  "    out.write('        return 0\\n')",
  "    out.write('    fi\\n')",
  "    out.write('    echo \"  cloning $name @ ${commit:0:8}\"\\n')",
  ...
  "                seen.add(commit)",
  "                out.write(f'clone_module \"{module.id}\" \"{url}\" \"{commit}\"\\n')",
  "    if bench.metric_collectors:",
  "        out.write('\\n# Metric collectors\\n')",
  "        for mc in bench.metric_collectors:",
  ...
  "            if commit not in seen:",
  "                seen.add(commit)",
  "                out.write(f'clone_module \"{mc.id}\" \"{url}\" \"{commit}\"\\n')",
