...
const PREVIEW_PY = [
  ...
  "                # shell — shebang-aware dispatch matching production behaviour",
  "                input_args = ' '.join(",
  "                    f'--{e.replace(\".\", \"_\")} {{input.{_ikey(e)}}}'",
  "                    for e in sinputs",
  "                )",
  "                cli_frag = '{params.cli_args}' if combo else ''",
  "                run_args = (f'--output_dir $(dirname {{output[0]}}) --name {module.id}'",
  "                            + (f' {input_args}' if input_args else '')",
  "                            + (f' {cli_frag}' if cli_frag else ''))",
  "                out.write('    shell:\\n        \"\"\"\\n')",
  "                out.write('        mkdir -p $(dirname {output[0]}) $(dirname {log})\\n')",
  "                out.write('        exec > >(tee {log}) 2>&1\\n')",
  "                out.write('        cd {params.module_dir}\\n')",
  "                out.write('        ep={params.entrypoint}\\n')",
  "                out.write('        if head -1 \"$ep\" | grep -q \"^#!\"; then\\n')",
  "                out.write('            chmod +x \"$ep\"\\n')",
  "                out.write(f'            ./$ep {run_args}\\n')",
  "                out.write('        else\\n')",
  "                out.write(f'            python3 \"$ep\" {run_args}\\n')",
  "                out.write('        fi\\n')",
  "                out.write('        \"\"\"\\n\\n')",
  "",
  "    # metric collectors",
